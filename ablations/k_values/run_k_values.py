...
def _output_dir(model_name, top_k):
    return os.path.join(
        config.get_results_dir(model_name),
        "ablations", "k-values", f"k_{top_k}", "raw_outputs",
    )
...
def _load_subset_cache(condition_id, top_k):
...
def run_condition(condition_id, cond_info, queries_df, generator, top_k, model_name=None):
    output_dir = _output_dir(model_name, top_k)
    ...
    if os.path.exists(output_path):
        ...
        if len(done_df) >= len(queries_df):
            logger.info("k=%s condition %s already complete (%s rows).", top_k, condition_id, len(done_df))
            return done_df
        already_done = set(done_df["query_id"].astype(str).tolist())
        existing_results = done_df.to_dict("records")
        logger.info("k=%s condition %s resuming with %s rows.", top_k, condition_id, len(already_done))
    ...
    if retriever_type != "baseline":
        subset_cache = _load_subset_cache(condition_id, top_k)

    retriever = None
    if retriever_type != "baseline" and subset_cache is None:
        logger.info("  No subset cache for %s (k=%s) — using live retrieval", condition_id, top_k)
        retriever = _get_retriever(retriever_type, corpus_name)

    remaining = len(queries_df) - len(already_done)
    logger.info("k=%s condition %s: %s remaining", top_k, condition_id, remaining)
    ...
    if use_two_phase and remaining > 0:
        ...
        if subset_cache is not None:
            for _, row in remaining_df.iterrows():
                qid     = str(row["id"])
                context = subset_cache.get(qid)
                all_prepared.append({
                    "query_id": row["id"], "query": str(row["query"]),
                    "reference": str(row.get("reference", "")),
                    "user_type": user_type, "context": context, "retrieval_scores": "[]",
                })
        else:
            for _, row in tqdm(remaining_df.iterrows(), total=remaining,
                               desc=f"k={top_k} Retrieving {condition_id}"):
                query = str(row["query"])
        ...
        for start in range(0, len(all_prepared), gen_subbatch):
            ...
            for prep, gen_out in zip(sub, generated):
                all_results.append({
                    ...
                    "top_k": top_k, "context": prep["context"] or "",
                    "response": gen_out["response"], "retrieval_scores": prep["retrieval_scores"],
        ...
        for _, row in tqdm(queries_df.iterrows(), total=len(queries_df), desc=f"k={top_k} {condition_id}"):
            qid = str(row["id"])
    ...
    logger.info("k=%s condition %s complete -> %s", top_k, condition_id, output_path)
    return final_df


def merge_results(model_name=None, top_k=None):
    k_values = [top_k] if top_k is not None else K_VALUES
    for k_value in k_values:
        raw_dir = _output_dir(model_name, k_value)
        all_dfs = []
        for cid in CONDITION_IDS:
            ...
            if os.path.exists(path):
                ...
                if len(df) > 0:
                    all_dfs.append(df)
        if not all_dfs:
            logger.warning("No outputs found for model=%s k=%s", model_name, k_value)
            continue
...
def run_k_ablation(model_name=None, condition_ids=None, k_values=None, use_vllm=False):
    ...
    for k_value in k_values:
        logger.info("=" * 60)
        logger.info("Running k=%s", k_value)
        logger.info("=" * 60)
        for cid in condition_ids:
            cond_info  = ext_config.EXTENDED_CONDITIONS[cid]
            queries_df = eval_sets[cond_info["queries"]]
            t0 = time.time()
            run_condition(cid, cond_info, queries_df, generator, k_value, model_name=model_name)
            logger.info("k=%s condition %s completed in %.1fs", k_value, cid, time.time() - t0)
        merge_results(model_name=model_name, top_k=k_value)
    logger.info("k-values ablation completed in %.1fs", time.time() - total_t0)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run retrieval top-k ablations on ablation_subset.")
